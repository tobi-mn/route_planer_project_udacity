...
RoutePlanner::RoutePlanner(RouteModel &model, float start_x, float start_y, float end_x, float end_y) : m_Model(model)
{
    ...
    RouteModel::Node &start_node_ = model.FindClosestNode(start_x, start_y);
    RouteModel::Node &end_node_ = model.FindClosestNode(end_x, end_y);
...
{
    const float distance_to_goal = node->distance(*end_node);
    return distance_to_goal;
}

float RoutePlanner::CalculateGValue(RouteModel::Node const *node)
{
    const float distance_to_start = node->parent->g_value + node->distance(*node->parent);
...
void RoutePlanner::AddNeighbors(RouteModel::Node *current_node)
{
    ...
    for (auto node : current_node->neighbors)
    {
        if (node->visited == false)
        {
            ...
            node->h_value = 1.0 * CalculateHValue(node);
            node->g_value = 1.0 * CalculateGValue(node);
